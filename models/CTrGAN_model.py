...
class CTrGANModel(BaseModel):
    ...
    def forward(self):
        ...
        self.real_A0_RGB = self.real_A0[:,0:4,:,:]
        self.real_A0_IUV = self.real_A0[:,4:,:,:]
        
        self.real_B0 = Variable(self.input_B0)
        self.real_B0_RGB = self.real_B0[:,0:4,:,:]
        self.real_B0_IUV = self.real_B0[:,4:,:,:]
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            idt_A0_RGB, idt_A0_IUV = util.split_tensor_to_IUV_and_RGB(idt_A0)
            loss_idt_A_RGB = (self.criterionIdt(idt_A0_RGB, self.real_B0_RGB)) * lambda_B * lambda_idt
            loss_idt_A_IUV = (self.criterionIdt(idt_A0_IUV, self.real_B0_IUV)) * lambda_B * lambda_idt
            
            # G_B should be identity if real_A is fed.
            B_Query = self.netF_B(self.real_A0)
            B_Query = self.Transformer_B(A_Keys, B_Query) if self.use_transformer else B_Query
            idt_B0 = self.netH_B(B_Query)
            idt_B0_RGB, idt_B0_IUV = util.split_tensor_to_IUV_and_RGB(idt_A0)
            loss_idt_B_RGB = (self.criterionIdt(idt_B0_RGB, self.real_A0_RGB)) * lambda_B * lambda_idt
            loss_idt_B_IUV = (self.criterionIdt(idt_B0_IUV, self.real_A0_IUV)) * lambda_B * lambda_idt

            self.idt_A = idt_A0.data
            self.idt_B = idt_B0.data
            self.loss_idt_A = loss_idt_A_RGB.item() + loss_idt_A_IUV.item()
            self.loss_idt_B = loss_idt_B_RGB.item() + loss_idt_B_IUV.item()
        else:
            loss_idt_A = 0
            loss_idt_B = 0
            self.loss_idt_A = 0
        ...
        A_Query = self.Transformer_A(B_Keys,A_Query) if self.use_transformer else A_Query
        rec_B0 = self.netH_A(A_Query)
        rec_B0_RGB, rec_B0_IUV = util.split_tensor_to_IUV_and_RGB(rec_B0)
        loss_cycle_B0_RGB = self.criterionCycle(rec_B0_RGB, self.real_B0_RGB) * lambda_B
        loss_cycle_B0_IUV = self.criterionCycle(rec_B0_IUV, self.real_B0_IUV) * lambda_B
        loss_cycle_B0 = (loss_cycle_B0_RGB + loss_cycle_B0_IUV)

        B_Query = self.netF_B(fake_B0)
        B_Query = self.Transformer_B(A_Keys, B_Query) if self.use_transformer else B_Query
        rec_A0 = self.netH_B(B_Query)
        rec_A0_RGB, rec_A0_IUV = util.split_tensor_to_IUV_and_RGB(rec_A0)
        loss_cycle_A0_RGB = self.criterionCycle(rec_A0_RGB, self.real_A0_RGB) * lambda_B
        loss_cycle_A0_IUV = self.criterionCycle(rec_A0_IUV, self.real_A0_IUV) * lambda_B
        loss_cycle_A0 = (loss_cycle_A0_RGB + loss_cycle_A0_IUV)
        ...
        if self.use_perceptual_loss:
            loss_G_perc_A0_RGB = self.perceptual_loss(rec_A0[:,0:3] , self.real_A0[:,0:3])
            loss_G_perc_A0_IUV = self.perceptual_loss(rec_A0[:,4:] , self.real_A0[:,4:])
            loss_G_perc_A0A = self.perceptual_loss(rec_A0[:,3:4], self.real_A0[:, 3:4])
            loss_G_perc_A0 = loss_G_perc_A0_RGB + loss_G_perc_A0_IUV + loss_G_perc_A0A
            
            loss_G_perc_B0_RGB = self.perceptual_loss(rec_B0[:,0:3] , self.real_B0[:,0:3])
            loss_G_perc_B0_IUV = self.perceptual_loss(rec_B0[:,4:] , self.real_B0[:,4:])
            loss_G_perc_B0A = self.perceptual_loss(rec_B0[:,3:4], self.real_B0[:, 3:4])
            loss_G_perc_B0 = loss_G_perc_B0_RGB + loss_G_perc_B0_IUV + loss_G_perc_B0A

            loss_G += loss_G_perc_B0 + loss_G_perc_A0
        loss_G.backward()
    ...
    def get_current_visuals(self):
        real_A0_RGB = util.tensor2im(self.real_A0_RGB)
        real_A0_IUV = util.tensor2im(self.real_A0_IUV)
        
        real_B0_RGB = util.tensor2im(self.real_B0_RGB)
        real_B0_IUV = util.tensor2im(self.real_B0_IUV)
        
        # fake_B0F = util.tensor2iuv(self.fake_B0[0:1])
        # fake_B0L = util.tensor2iuv(self.fake_B0[-1:])
        
        fake_B0_RGB, fake_B0_IUV = util.split_tensor_to_IUV_and_RGB(self.fake_B0)
        fake_B0_RGB = util.tensor2im(fake_B0_RGB)
        fake_B0_IUV = util.tensor2im(fake_B0_IUV)
        
        fake_A0_RGB, fake_A0_IUV = util.split_tensor_to_IUV_and_RGB(self.fake_A0)
        fake_A0_RGB = util.tensor2im(fake_A0_RGB)
        fake_A0_IUV = util.tensor2im(fake_A0_IUV)
        ...
        ret_visuals = OrderedDict([('real_A0_RGB', real_A0_RGB),('real_A0_IUV', real_A0_IUV), 
                                   ('real_B0_RGB', real_B0_RGB),('real_B0_IUV', real_B0_IUV),
                                   ('fake_B0_RGB', fake_B0_RGB), ('fake_B0_IUV', fake_B0_IUV),
                                   ('fake_A0_RGB', fake_A0_RGB), ('fake_A0_IUV', fake_A0_IUV),
                                   ('rec_B0',rec_B0),('rec_A0',rec_A0)])
    ...
    def get_current_visuals_wandb(self):
        real_A0 = util.tensor2iuv(self.input_A0) #input image a0
        fake_B0 = util.tensor2iuv(self.fake_B0)  #output image b0 (reconstructed using query of a0)
        real_B0 = util.tensor2iuv(self.input_B0) #input image a0
        fake_A0 = util.tensor2iuv(self.fake_A0)  #output image a0 (reconstructed using query of b0)

        fake_B0_I = util.tensor2im(self.fake_B0[:,2,:,:].unsqueeze(0))
        fake_A0_I = util.tensor2im(self.fake_A0[:,2,:,:].unsqueeze(0))
        rec_B0 = util.tensor2iuv(self.rec_B0) #the reconstructed image b0 from fake a0
        rec_A0 = util.tensor2iuv(self.rec_A0) #the reconstructed image a0 from fake b0
        ...
        if self.opt.isTrain and self.opt.identity > 0.0:
            ret_visuals['others']['idt_A'] = util.tensor2iuv(self.idt_A) #reconstructed a0 from query of fake a0
            ret_visuals['others']['idt_B'] = util.tensor2iuv(self.idt_B) #reconstructed b0 from query of fake b0
