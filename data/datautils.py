...
def prepare_images(A_img, fineSize):
    ...
    return A0
'''
def load_packets_utils(image, img_type, load_size, transform, fine_size,offset=None):
    AP_img = resize_img(image, load_size, transform, img_type)
    AP0, offset = prepare_images(AP_img, fine_size, offset)
    AP0 = AP0.unsqueeze(0)
    return AP0, offset
...
def resize_img(img, loadSize, transform, itype='img'):
    ...
    if itype in ['hm_iuv','iuv1']:
        ...
        img_ = torch.tensor(np.concatenate((luv,li),axis=2))
...
def load_centroids_images_utils(img, img_mode, transform, offset = None):
    loadSize = fineSize = img.size[0]
    out_img = resize_img(img, loadSize, transform, itype=img_mode)
    out_img, offset = prepare_images(out_img, fineSize, offset)
    out_img = out_img.unsqueeze(0)
    return out_img, offset

def load_centroids_images(itemlist, iuv_mode ='iuv1'):
    rgba_transform_list = [transforms.ToTensor(),
                           transforms.Normalize((0.5, 0.5, 0.5, 0.5),
                                                (0.5, 0.5, 0.5, 0.5))]  # SM added layer for alpha
    rgba_transform = transforms.Compose(rgba_transform_list)
    
    rgb_transform_list = [transforms.ToTensor(),
                          transforms.Normalize((0.5, 0.5, 0.5),
                                               (0.5, 0.5, 0.5))]
    rgb_transform = transforms.Compose(rgb_transform_list)

    imglist = []
    for f in itemlist:
        if isinstance(f, tuple) == False:
            AP_img = Image.open(f).convert('RGBA')  # SM - changed to RGBA (instead of RGB)
            AP0 = load_centroids_images_utils(AP_img, 'img', rgba_transform)
        else:
            rgb_image = Image.open(f[0]).convert('RGBA')
            rgb, offset = load_centroids_images_utils(rgb_image, 'img', rgba_transform)
            iuv_image = Image.open(f[1]).convert('RGB')
            iuv, _ = load_centroids_images_utils(iuv_image, 'iuv1', rgb_transform, offset)
...
def concat_rgb_with_iuv(rgb_list, iuv_list):
    concatenated_list = []
    for j in range(len(rgb_list)):
        temp_list = []
        for rgb, iuv in zip(rgb_list[j], iuv_list[j]):
            temp_list.append((rgb, iuv))
        concatenated_list.append(temp_list)
